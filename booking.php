...
$stmt = $db->prepare("
    SELECT b.datefrom, b.dateto, c.customername, r.roomname
    FROM booking b
...
while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
    ...
    for ($d = max($start, $firstDayOfMonth); $d <= min($end, strtotime($lastDayStr)); $d += 86400) {
        $day = date('j', $d);
        $bookingsByDay[$day][] = $row['customername'] . " (" . $row['roomname'] . ")";
    }
}
...
.booking { font-size:0.8em; margin-top:4px; color:#fff; }
    ...
    <section class="content">
        ...
        <div class="calendar">
            ...
            foreach ($weekdays as $day) {
                echo '<div class="header">' . esc($day) . '</div>';
            }
            ...
            for ($day=1; $day<=$daysInMonth; $day++) {
                ...
                if (isset($bookingsByDay[$day])) {
                    foreach ($bookingsByDay[$day] as $b) {
                        echo '<div class="booking">' . esc($b) . '</div>';
                    }
                }
                echo '</div>';
            }
